...
mkdir build
cmake -GNinja -DWITH_MPI=TRUE -Dcatalyst_DIR=<installation path of catalyst>/lib/cmake/catalyst-2.0
ninja
```
...
export CATALYST_IMPLEMENTATION_PATH=<paraview build directory>/lib/catalyst
mpiexec -np 2 ./lulesh2.0 -x lulest_state.py -p -i 1000 -s 50
```
